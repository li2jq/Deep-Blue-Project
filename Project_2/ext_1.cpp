...
int main(int argc, char* argv[])
{
    if(argc != 3)
    {
        std::cout << "Program format is: ext_1 file_name.txt int_num" << endl;
        return 1;
    }

    int num_base = std::stoi(argv[2]);

    if (num_base < 1 || num_base > 36)
    {
        std::cout << "Invalid number base!" << endl;
        return 1;
    }
    ...
    if (add_1.sign_part == add_2.sign_part)
    {
        ...
        {
            int remain_num = ( add_1.num_part[i] + add_2.num_part[i] + carry ) % num_base;
            if (i == cycle_num)
            {
                if (remain_num == 0 )
                {
            ...
            carry = ( add_1.num_part[i] + add_2.num_part[i] + carry ) / num_base;
        }
    }
    ...
    else // add_1.sign_part != add_2.sign_part
    {
        ...
        for (int i = 0; i < cycle_num; i++)
        {
            ...
            {
                int sub_res = (larger_one.num_part[i] - borrow_bit) + num_base - small_one.num_part[i];
                res.num_part.push_back(sub_res);
                borrow_bit = 1;
            } 
        }
